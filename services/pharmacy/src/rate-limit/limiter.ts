...
/**
 * Per-user rate limiting (docs/API.md: pharmacy 10/hour). One DynamoDB item
 * per user+route+hour-bucket, incremented atomically with a TTL so old
 * buckets expire on their own. Stored in the Reference table under the same
 * RATE# prefix lane C uses (services/scan/src/rate-limit/limiter.ts) - same
 * shape, kept as a local copy so this lane doesn't depend on C's internals.
 */
