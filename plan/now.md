...
- **Testset size (E):** 15/30 strips, 6/10 bills labelled (bills went from 1/10 to 6/10 this session — two team members provided their own real, redacted Tata 1mg pharmacy invoices, `testset/sources.md`'s "bills 002-006" note). Strips still need actual human-shot photos of real Indian medicine strips (`testset/README.md`); internet sourcing for strips has been exhausted (`testset/sources.md`'s "Coverage gaps" section). A fresh full harness run against the now-6-bill testset is queued but hasn't completed yet — see `plan/tasks/E-accuracy-harness.md` Handoff (hit a real per-user hourly scan rate limit mid-run, plus a real Lambda-timeout finding on a 3-line-item bill worth lane C's attention).
