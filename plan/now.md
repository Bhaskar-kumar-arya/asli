...
| Lane | Note |
|---|---|
| A1, A2, B, C, D1, D2, D3, F, G1, G2, H, I, S | Can build against `contracts-v1`-pending schemas now (packages/contracts is code-complete, not yet tag-frozen - see T02 Handoff). Their own `cdk deploy` still needs `dev-shared` deployed first for SSM imports to resolve at deploy time (synth-only work is unblocked already). A3 (PDF/Textract fallback) can likely drop to P2 - T01 confirmed `ENDPOINT_OK` with strong evidence. |
