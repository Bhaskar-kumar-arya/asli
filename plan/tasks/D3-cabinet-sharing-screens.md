...
**Remaining:**
- **Verified against real `int` (2026-09-19, follow-up session)** with a real signed-in Playwright browser session against the live Amplify Hosting site: `CabinetPage` and `MedicineDetailPage` both load real seeded data (`cab-demo-mom-001`, both a `FLAGGED` and a `NO_ALERT_FOUND` medicine) correctly, with the right wording and CDSCO source citation - see `submission/LEARNING_LOG.md`'s 2026-09-19 entry for the three bugs found and fixed along the way (API Gateway CORS, a double `/v1/v1` path prefix in this lane's own `api/client.ts`, and `api/auth.ts` - the placeholder auth stub this lane's own comment said to delete once D1 shipped real sign-in - never actually being replaced). Two-account invite share and the real push-notification deep link are still unverified end-to-end (would need two real signed-in sessions/devices).
- **Home screen integration: done (2026-09-19, follow-up session).** `HomeMedicineList` was built and tested but never actually mounted - `apps/web/src/app/HomeScreen.tsx` had a static "medicines will show up here" placeholder that could never show anything. Now renders `<HomeMedicineList />` directly.
