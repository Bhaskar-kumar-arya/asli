...
- **"Continue as guest" button, i.e. guest mode (2026-09-20, cross-lane, `apps/web/src/auth`, requested by the human):** `SignInScreen.tsx` now has a button that signs in as the seeded demo account (`demoAccount.ts`, overridable via `VITE_DEMO_EMAIL`/`VITE_DEMO_PASSWORD`), with 2 new tests in `SignInScreen.test.tsx`. `tsc`, `eslint`, `pnpm --filter @asli/web test` (76/76) and `build` pass. Not live until pushed and Amplify rebuilds. README, WRITEUP, FORM_ANSWERS and DEMO_SCRIPT now tell judges to press it.
...
- ~~Amplify SPA rewrite rule~~ — done 2026-09-20 (this session had AWS CLI access the one that found it lacked). The existing `404-200` catch-all wasn't firing because Amplify's underlying S3 hosting 301-redirects an extension-less path to a trailing slash before the 404 rule gets a chance to run. Replaced it via `aws amplify update-app --custom-rules` with the exact rule this line used to recommend (source `</^[^.]+$|\.(?!(css|gif|ico|jpg|js|png|txt|svg|woff|woff2|ttf|map|json|webp)$)([^.]+$)/>`, target `/index.html`, type `200 (Rewrite)`). Verified live: `/insights`, `/dashboard`, `/sign-in` all return 200 directly now.
