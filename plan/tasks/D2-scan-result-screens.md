...
- Type-ahead on both "Medicine name" and "Manufacturer" (`components/SuggestField.tsx`, generic; `lib/typeahead.ts`, the shared ranking engine; `lib/medicineNames.ts` and `lib/manufacturerNames.ts`, thin per-field dictionaries), shared by manual entry and confirm-details via `MedicineIdentityForm`. ARIA `combobox`/`listbox` suggestions ranked prefix-match → substring → typo-tolerant fuzzy match (capped Levenshtein), boosted by this device's own recent/frequent searches (`localStorage`, keyed separately per field, never sent to a server - no cross-user "popular searches" backend exists, so this is honestly per-device personalization, not global popularity). Doesn't auto-overwrite typed text - a typo'd or off-dictionary name just shows as a pickable suggestion, so both fields stay free text as the contract already allows. Medicine-name dictionary is seeded from the CDSCO product names already in `packages/contracts/fixtures/flagged-batches.json` plus a few common OTC brand names. Manufacturer dictionary is deliberately **not** drawn from that same fixture's `manufacturerRaw` values (those are synthetic test data invented for fixtures, not real companies - suggesting them would misleadingly imply real manufacturers are commonly flagged); it's a short list of real, well-known Indian pharma manufacturers instead. 6 new tests in `MedicineIdentityForm.test.tsx` + 6 in `lib/medicineNames.test.ts` + 5 in new `lib/manufacturerNames.test.ts` (74 total across the whole app now).
**Remaining:**
