...
- "Medicine name" type-ahead (`components/MedicineNameField.tsx` + `lib/medicineNames.ts`), shared by manual entry and confirm-details via `MedicineIdentityForm`: ARIA `combobox`/`listbox` suggestions ranked prefix-match → substring → typo-tolerant fuzzy match (capped Levenshtein), seeded from the CDSCO product names already in `packages/contracts/fixtures/flagged-batches.json` plus a few common OTC brand names, boosted by this device's own recent/frequent searches (`localStorage`, never sent to a server - no cross-user "popular searches" backend exists, so this is honestly per-device personalization, not global popularity). Doesn't auto-overwrite typed text - a typo'd or off-dictionary name just shows as a pickable suggestion, so `productName` stays free text as the contract already allows. 3 new tests in `MedicineIdentityForm.test.tsx` + 6 in new `lib/medicineNames.test.ts` (29 total for D2 now, 66 across the whole app).
**Remaining:**
